...
# 法鼓文理学院 YBh 项目「网络再发布」实践报告  
（基于 2026-01 公开仓库数据）[^kimi]
...
1. MCP（Model Context Protocol）  
   计划把 YBh XML 的 `//seg[@lang="san"]` 节点暴露为「只读 MCP 端点」，让 Claude Desktop 直接检索梵文原句，减少上下文长度。  
2. 自动化翻译辅助  
   - 新建 `claude-skills/translate-san-zh` 目录，封装 Prompt 模板；  
   - 通过 GitHub Actions 调用 Claude 3.5 API，对每次新增的 `.md` 生成「机器翻译草稿」并自动插入 `> [!AI-draft]` callout，供后期人工校订。  
3. 知识问答  
